...
function SectionTitle({ icon, title, subtitle, t }) {
  return (
    <div style={{ marginBottom:28 }}>
      ...
      {subtitle && <p style={{ margin:"0 0 0 30px", fontSize:13, color:t.textMuted }}>{subtitle}</p>}
    </div>
  );
}
      ...
      <div style={{ width:"100%", maxWidth:420 }}>
        ...
        <div style={{ textAlign:"center", marginBottom:32 }}>
          <div style={{
            ...
            <Icon name="calendarEvent" size={26} color="#fff" />
          </div>
          <div style={{ fontFamily:"'Playfair Display',serif", fontWeight:800, fontSize:24, color:t.text }}>Timetable For All</div>
          <div style={{ fontSize:12, color:t.textMuted, marginTop:4, letterSpacing:"0.08em", textTransform:"uppercase" }}>Kenyan CBC · School Scheduler</div>
        </div>

        <Card t={t} style={{ padding:28 }}>
          {/* Tab switch */}
          <div style={{ display:"flex", background:t.card, borderRadius:9, padding:3, marginBottom:22, gap:3 }}>
            {[["login","Sign In"],["signup","Create Account"]].map(([m,label])=>(
              <button key={m} onClick={()=>{ setMode(m); setError(""); setSuccess(""); }} style={{
                flex:1, padding:"9px", borderRadius:7, border:"none", cursor:"pointer",
                background:mode===m?t.accent:"transparent",
                color:mode===m?"#fff":t.textMuted,
                fontWeight:700, fontSize:13, fontFamily:"'DM Sans',sans-serif", transition:"all 0.18s"
              }}>{label}</button>
            ))}
          </div>

          {mode === "signup" && (
            <Input label="School Name" value={schoolName} onChange={setSchoolName} placeholder="e.g. Nairobi Academy" t={t} />
          )}
          <Input label="Email Address" value={email} onChange={setEmail} placeholder="admin@school.ac.ke" type="email" t={t} />
          <Input label="Password" value={password} onChange={setPassword} placeholder="••••••••" type="password" t={t} />

          {error && (
            <div style={{ background:t.danger+"18", border:`1px solid ${t.danger}44`, borderRadius:8, padding:"10px 14px", color:t.danger, fontSize:13, marginBottom:14 }}>
              {error}
            </div>
          )}
          {success && (
            <div style={{ background:t.successGlow, border:`1px solid ${t.success}44`, borderRadius:8, padding:"10px 14px", color:t.success, fontSize:13, marginBottom:14 }}>
              {success}
            </div>
          )}
    ...
    <div>
      ...
      <Card t={t} style={{ padding:16 }}>
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
          {slots.map(s=>(
            <div key={s.id} style={{ padding:"5px 12px", borderRadius:20, fontSize:11, fontWeight:600,
              background:(typeColor[s.type]||t.accent)+"18", border:`1px solid ${(typeColor[s.type]||t.accent)}33`,
            ...
            {slots.map(sl=>(
              <div key={sl.id} style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap" }}>
                ...
                <input value={sl.label} onChange={e=>setSlots(p=>p.map(x=>x.id===sl.id?{...x,label:e.target.value}:x))}
                  style={{ flex:1, minWidth:80, padding:"6px 10px", borderRadius:7, background:t.inputBg, border:`1px solid ${t.border}`, color:t.text, fontSize:12 }} />
          ...
          <Card t={t} style={{ marginBottom:12, padding:14 }}>
            ...
            <div style={{ display:"flex", gap:6, marginBottom:8 }}>
              <select value={grade} onChange={e=>setGrade(e.target.value)}
                style={{ flex:1, padding:"8px 9px", borderRadius:7, background:t.inputBg, border:`1px solid ${t.border}`, color:t.text, fontSize:13 }}>
                {["Grade 1","Grade 2","Grade 3","Grade 4","Grade 5","Grade 6","Grade 7","Grade 8","Grade 9"].map(g=><option key={g}>{g}</option>)}
              </select>
              <select value={stream} onChange={e=>setStream(e.target.value)}
                style={{ width:52, padding:"8px 6px", borderRadius:7, background:t.inputBg, border:`1px solid ${t.border}`, color:t.text, fontSize:13 }}>
          ...
          {active ? (
            <>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:14, flexWrap:"wrap", gap:10 }}>
                <div>
                  <div style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, fontSize:17, color:t.text }}>{activeClass?.grade} {activeClass?.stream}</div>
                ...
                <div style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap" }}>
                  <select value={preset} onChange={e=>setPreset(e.target.value)}
                    style={{ padding:"8px 10px", borderRadius:7, background:t.inputBg, border:`1px solid ${t.border}`, color:t.text, fontSize:12 }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
                  ...
                  <Card key={subj.id} t={t} style={{ padding:"10px 14px" }}>
                    <div style={{ display:"flex", gap:10, alignItems:"center", flexWrap:"wrap" }}>
                      ...
                      <input value={subj.name} onChange={e=>setSubjects(p=>p.map(s=>s.id===subj.id?{...s,name:e.target.value}:s))}
                        style={{ flex:1, minWidth:120, padding:"7px 11px", borderRadius:7, background:t.inputBg, border:`1px solid ${t.border}`, color:t.text, fontSize:13, fontWeight:600 }} />
                ...
                {activeSubjects.length===0 && (
                  <div style={{ textAlign:"center", padding:"36px 0", color:t.textFaint, fontSize:13 }}>Load a CBC preset or add subjects manually</div>
                )}
              </div>
            </>
          ) : (
        ...
        <div>
          <Card t={t} style={{ marginBottom:12, padding:14 }}>
            ...
            <input value={newName} onChange={e=>setNewName(e.target.value)}
              ...
              style={{ width:"100%", padding:"9px 11px", borderRadius:7, background:t.inputBg, border:`1px solid ${t.border}`, color:t.text, fontSize:13, marginBottom:8, boxSizing:"border-box" }} />
            <Btn variant="primary" style={{ width:"100%", justifyContent:"center" }} onClick={()=>{
...
function StepGenerate({ school, classes, subjects, teachers, slots, timetable, setTimetable, t }) {
    ...
    DAYS.forEach(day=>{
      lessonSlots.forEach((_,i)=>{
        ...
        classes.forEach(cls=>{
          ...
          if(cell?.teacherId){
            if(tm[cell.teacherId]){ const tc=teachers.find(x=>x.id===cell.teacherId); found.push(`${tc?.name||"Teacher"} — ${day} slot ${i+1}`); }
  ...
  const printClass = () => {
    ...
    slots.forEach((slot,idx)=>{
      if(slot.type!=="lesson"){ rows+=`<tr><td colspan="6" style="background:#fffde7;color:#f57c00;text-align:center;padding:6px;font-size:12px">${slot.start} — ${slot.label}</td></tr>`; return; }
      const li=slots.slice(0,idx+1).filter(s=>s.type==="lesson").length-1;
      ...
      DAYS.forEach(day=>{
        ...
        const subj=cell?subjects.find(s=>s.id===cell.subjectId):null;
        const tch=cell?teachers.find(tc=>tc.id===cell.teacherId):null;
        row+=subj?`<td style="padding:7px;text-align:center;border:1px solid #eee;background:${subj.color}18"><div style="font-weight:800;font-size:11px;color:${subj.color}">${subj.name}${cell.isDouble&&cell.doubleStart?" ×2":""}</div>${tch?`<div style="font-size:10px;color:#888">${tch.name}</div>`:""}</td>`:`<td style="border:1px solid #eee"></td>`;
      });
      rows+=row+"</tr>";
    });
  ...
  return (
    <div>
      <SectionTitle icon="cpu" title="Generate Timetable" subtitle="Run the conflict-free solver, review compliance, and export" t={t} />
        ...
        <Card t={t} style={{ padding:18, minWidth:240 }}>
          ...
          {timetable && (
            <div style={{ display:"flex", gap:7 }}>
              ...
              <Btn size="sm" variant="secondary" onClick={()=>setVersions(p=>[...p,{label:`${school.term} v${p.length+1}`,ts:new Date().toLocaleString(),published:false}])} t={t}>
                <Icon name="save" size={13} color={t.textMuted} /> Save Version
              </Btn>
            </div>
          )}
        </Card>
              ...
              <div key={i} style={{ padding:"8px 14px", borderRadius:9, background:v.published?t.successGlow:t.card, border:`1px solid ${v.published?t.success:t.border}`, fontSize:12 }}>
                ...
                {!v.published && (
                  <button onClick={()=>setVersions(p=>p.map((vv,ii)=>({...vv,published:ii===i})))}
                    style={{ background:"none", border:"none", color:t.success, cursor:"pointer", fontSize:11, fontWeight:700, padding:"2px 0" }}>Publish</button>
                )}
        ...
        <>
          <div style={{ display:"flex", gap:8, marginBottom:14, flexWrap:"wrap", alignItems:"center" }}>
            <div style={{ display:"flex", background:t.card, borderRadius:9, border:`1px solid ${t.border}`, padding:3, gap:2 }}>
              {[["class","Class View","grid"],["teacher","Teacher View","person"]].map(([v,label,ic])=>(
                <button key={v} onClick={()=>setView(v)} style={{
                  ...
                  fontWeight:600, fontSize:12, display:"flex", alignItems:"center", gap:6, transition:"all 0.15s"
                }}>
            ...
            {view==="class" && (
              <select value={selClass||""} onChange={e=>setSelClass(e.target.value)}
                style={{ padding:"8px 12px", borderRadius:8, background:t.inputBg, border:`1px solid ${t.border}`, color:t.text, fontSize:13 }}>
          ...
          <Card t={t} style={{ padding:0, overflow:"hidden" }}>
            ...
            <div style={{ padding:14 }}>
              {view==="class" && selClass && <ClassGrid timetable={timetable} classId={selClass} subjects={subjects} teachers={teachers} slots={slots} t={t} />}
              {view==="teacher" && selTeacher && <TeacherGrid timetable={timetable} teacher={teachers.find(tc=>tc.id===selTeacher)} classes={classes} subjects={subjects} slots={slots} t={t} />}
            </div>
          </Card>
        </>
      )}
...
export default function App() {
  ...
  const saveData = useCallback(async () => {
    ...
    const payload = { school, slots, classes, subjects, teachers, timetable };
    let error;
    if (projectId) {
      ({ error } = await supabase
        .from("timetable_projects")
        .update({ data: payload, school_name: school.name || "Unnamed School", updated_at: new Date().toISOString() })
        .eq("id", projectId));
    } else {
      const { data, error: e } = await supabase
        .from("timetable_projects")
        .insert({ user_id: session.user.id, school_name: school.name || "Unnamed School", data: payload })
        .select()
        .single();
      error = e;
  ...
  }, [session, projectId, school, slots, classes, subjects, teachers, timetable]);

  useEffect(() => {
    if (!session) return;
    const timer = setTimeout(saveData, 2000);
    return () => clearTimeout(timer);
  }, [school, slots, classes, subjects, teachers, timetable]);

  const handleSignOut = async () => {
    await supabase.auth.signOut();
    setSession(null);
    setProjectId(null);
    setSchool({ name:"", county:"", term:"Term 1 2026", lessonDuration:40 });
    setSlots(DEFAULT_SLOTS);
    setClasses([]); setSubjects([]); setTeachers([]); setTimetable(null);
  };
    ...
    <div style={{ minHeight:"100vh", background:t.bg, color:t.text, fontFamily:"'DM Sans',system-ui,sans-serif" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        @media(max-width:640px){.two-col{grid-template-columns:1fr !important}.hide-mobile{display:none !important}}
      `}</style>

      {/* ── HEADER ── */}
      <header style={{ background:t.navBg, borderBottom:`1px solid ${t.border}`, position:"sticky", top:0, zIndex:100, backdropFilter:"blur(14px)", WebkitBackdropFilter:"blur(14px)" }}>
        <div style={{ maxWidth:1200, margin:"0 auto", padding:"0 20px", display:"flex", alignItems:"center", height:58, gap:12 }}>
          <div style={{ display:"flex", alignItems:"center", gap:10, flex:1 }}>
            <div style={{ width:34, height:34, borderRadius:9, background:`linear-gradient(135deg,${t.accent},${t.success})`, display:"flex", alignItems:"center", justifyContent:"center", boxShadow:`0 0 18px ${t.accentGlow}`, flexShrink:0 }}>
              <Icon name="calendarEvent" size={17} color="#fff" />
            </div>
            <div>
              <div style={{ fontFamily:"'Playfair Display',serif", fontWeight:800, fontSize:16, color:t.text, lineHeight:1.1 }}>Timetable For All</div>
          ...
          {/* Save status */}
          {saveStatus && (
            <div style={{ fontSize:11, color:saveStatus==="saving"?t.textMuted:saveStatus==="saved"?t.success:t.danger, display:"flex", alignItems:"center", gap:4 }} className="hide-mobile">
              <Icon name={saveStatus==="saving"?"cloud":saveStatus==="saved"?"checkCircle":"warning"} size={12} color={saveStatus==="saving"?t.textMuted:saveStatus==="saved"?t.success:t.danger} />
          ...
          {/* User email */}
          <div className="hide-mobile" style={{ fontSize:11, color:t.textFaint }}>{session.user.email}</div>

          <button onClick={()=>setDark(p=>!p)} style={{ background:t.card, border:`1px solid ${t.border}`, borderRadius:8, padding:"7px 12px", cursor:"pointer", display:"flex", alignItems:"center", gap:7, color:t.textMuted, fontSize:12, fontWeight:600 }}>
            <Icon name={dark?"sun":"moon"} size={14} color={t.textMuted} />
            <span className="hide-mobile">{dark?"Light":"Dark"}</span>
          </button>

          <button onClick={handleSignOut} style={{ background:"none", border:`1px solid ${t.border}`, borderRadius:8, padding:"7px 12px", cursor:"pointer", display:"flex", alignItems:"center", gap:6, color:t.textMuted, fontSize:12, fontWeight:600 }}>
      ...
      {/* ── BOTTOM NAV (mobile) ── */}
      <div style={{ display:"none", position:"fixed", bottom:0, left:0, right:0, background:t.navBg, borderTop:`1px solid ${t.border}`, backdropFilter:"blur(14px)", WebkitBackdropFilter:"blur(14px)", zIndex:200 }} className="mobile-nav">
        <style>{`@media(max-width:640px){.mobile-nav{display:flex !important}}`}</style>
        {steps.map((s,i)=>(
          <button key={i} onClick={()=>setStep(i)} style={{ flex:1, padding:"10px 4px", background:"none", border:"none", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", gap:3, color:step===i?t.accent:t.textMuted, transition:"color 0.15s" }}>
